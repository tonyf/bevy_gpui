...
impl Window {
    ...
    /// Paints a host-owned texture through a stable, renderer-resolved ID.
    ///
    /// The ID and geometry are safe to copy into [`SceneSnapshot`]; no native
    /// texture or window object enters the scene transfer packet.
    pub fn paint_external_surface(&mut self, bounds: Bounds<Pixels>, id: crate::ExternalSurfaceId) {
        self.invalidator.debug_assert_paint();
        let scale_factor = self.scale_factor();
        self.next_frame
            .scene
            .insert_primitive(crate::ExternalSurface {
                order: 0,
    ...
    pub fn dispatch_event(&mut self, event: PlatformInput, cx: &mut App) -> DispatchEventResult {
        ...
        let pointer_was_captured = self.captured_hitbox.is_some();
        let is_pointer_event = event.mouse_event().is_some();
        ...
        DispatchEventResult {
            ...
            pointer_hit: is_pointer_event
                && (pointer_was_captured || !self.mouse_hit_test.ids.is_empty()),
        }
    }
