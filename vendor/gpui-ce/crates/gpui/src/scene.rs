...
impl Scene {
    ...
    /// Copies renderer-ready scene data into a sendable snapshot.
    ///
    /// Platform-native paint surfaces are deliberately rejected because they
    /// cannot safely cross a host render-world boundary. Hosts should register
    /// such textures through the external-surface API once available.
    pub fn snapshot(&self) -> Result<SceneSnapshot, SceneSnapshotError> {
        if !self.surfaces.is_empty() {
            return Err(SceneSnapshotError::PlatformSurface);
        }
        ...
        Ok(SceneSnapshot {
            ...
            external_surfaces: self.external_surfaces.clone(),
            backdrop_filters: self.backdrop_filters.clone(),
            filter_boundaries: self.filter_boundaries.clone(),
        })
    }
}

/// A renderer-ready GPUI scene that can cross a host render thread boundary.
#[derive(Clone, Debug, Default)]
pub struct SceneSnapshot {
    /// Ordered shadow primitives.
    pub shadows: Vec<Shadow>,
    /// Ordered quad primitives.
    pub quads: Vec<Quad>,
    /// Ordered path primitives.
    pub paths: Vec<Path<ScaledPixels>>,
    /// Ordered underline primitives.
    pub underlines: Vec<Underline>,
    /// Ordered monochrome atlas sprites.
    pub monochrome_sprites: Vec<MonochromeSprite>,
    /// Ordered subpixel atlas sprites.
    pub subpixel_sprites: Vec<SubpixelSprite>,
    /// Ordered color atlas sprites.
    pub polychrome_sprites: Vec<PolychromeSprite>,
    surface_markers: Vec<SnapshotSurface>,
    /// Ordered host-neutral textures resolved by the embedding renderer.
    pub external_surfaces: Vec<ExternalSurface>,
    /// Ordered backdrop filters.
    pub backdrop_filters: Vec<BackdropFilter>,
    /// Ordered content-filter boundaries.
    pub filter_boundaries: Vec<FilterBoundary>,
}

impl SceneSnapshot {
    /// Returns the scene primitives in painter order.
    pub fn batches(&self) -> impl Iterator<Item = PrimitiveBatch> + '_ {
        BatchIterator::new(
            &self.shadows,
            &self.quads,
...
/// The reason a [`Scene`] could not be transferred to a host renderer.
#[derive(Clone, Copy, Debug, Eq, PartialEq, thiserror::Error)]
pub enum SceneSnapshotError {
    /// The scene contains a platform-native surface that is not sendable.
    #[error("scene contains a platform-native paint surface")]
    PlatformSurface,
}
...
trait OrderedSurface {
    fn order(&self) -> DrawOrder;
}

impl OrderedSurface for PaintSurface {
    fn order(&self) -> DrawOrder {
        self.order
    }
}

impl OrderedSurface for SnapshotSurface {
    fn order(&self) -> DrawOrder {
        self.order
    }
}

impl<'a, S: OrderedSurface> BatchIterator<'a, S> {
    fn new(
        shadows: &'a [Shadow],
        quads: &'a [Quad],
        ...
        surfaces: &'a [S],
        external_surfaces: &'a [ExternalSurface],
        backdrop_filters: &'a [BackdropFilter],
        filter_boundaries: &'a [FilterBoundary],
    ) -> Self {
        Self {
            shadows_start: 0,
            shadows_iter: shadows.iter().peekable(),
            quads_start: 0,
            quads_iter: quads.iter().peekable(),
            paths_start: 0,
            paths_iter: paths.iter().peekable(),
            underlines_start: 0,
            underlines_iter: underlines.iter().peekable(),
            monochrome_sprites_start: 0,
            monochrome_sprites_iter: monochrome_sprites.iter().peekable(),
            subpixel_sprites_start: 0,
            subpixel_sprites_iter: subpixel_sprites.iter().peekable(),
            polychrome_sprites_start: 0,
            polychrome_sprites_iter: polychrome_sprites.iter().peekable(),
            surfaces_start: 0,
            surfaces_iter: surfaces.iter().peekable(),
            external_surfaces_start: 0,
            external_surfaces_iter: external_surfaces.iter().peekable(),
            backdrop_filters_start: 0,
            backdrop_filters_iter: backdrop_filters.iter().peekable(),
            filter_boundaries_start: 0,
            filter_boundaries_iter: filter_boundaries.iter().peekable(),
        }
    }
}
...
struct BatchIterator<'a, S: OrderedSurface> {
    shadows_start: usize,
    ...
    surfaces_iter: Peekable<slice::Iter<'a, S>>,
...
impl<S: OrderedSurface> Iterator for BatchIterator<'_, S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let mut orders_and_kinds = [
            ...
            (
                self.surfaces_iter.peek().map(|surface| surface.order()),
                PrimitiveKind::Surface,
            ),
            (
                self.external_surfaces_iter
                    .peek()
        ...
        match batch_kind {
            ...
            PrimitiveKind::ExternalSurface => {
                let surfaces_start = self.external_surfaces_start;
                let mut surfaces_end = surfaces_start + 1;
                ...
                self.external_surfaces_start = surfaces_end;
...
/// Stable identifier for a texture owned and resolved by an embedding host.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub struct ExternalSurfaceId(pub u64);

/// A host-neutral textured rectangle in a sendable scene snapshot.
#[derive(Clone, Copy, Debug)]
pub struct ExternalSurface {
    /// Painter order assigned by [`Scene::insert_primitive`].
    pub order: DrawOrder,
    /// Destination bounds in physical pixels.
    pub bounds: Bounds<ScaledPixels>,
    /// GPUI clipping mask.
    pub content_mask: ContentMask<ScaledPixels>,
...
mod tests {
    ...
    #[test]
    fn scene_snapshot_is_sendable_and_preserves_batch_order() {
        ...
        let batches: Vec<_> = snapshot
            ...
            .collect();
        assert_eq!(batches, vec!["quad", "external", "backdrop", "quad"]);
    }
}
