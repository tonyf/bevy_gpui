...
impl AppCell {
    ...
    #[doc(hidden)]
    #[track_caller]
    pub fn try_borrow(&self) -> Result<AppRef<'_>, std::cell::BorrowError> {
        if option_env!("TRACK_THREAD_BORROWS").is_some() {
            let thread_id = std::thread::current().id();
            eprintln!("borrowed {thread_id:?}");
        }
        Ok(AppRef(self.app.try_borrow()?))
    }
...
/// A GPUI application whose lifetime is driven by an external host event loop.
///
/// Unlike [`Application::run`], this handle never calls [`Platform::run`]. It
/// keeps GPUI's application allocation alive and provides explicit,
/// main-thread-only update and shutdown boundaries for hosts such as Bevy.
...
impl EmbeddedApplication {
    fn assert_main_thread(&self) -> Result<()> {
        if std::thread::current().id() != self.main_thread {
            return Err(anyhow!(
                "embedded GPUI applications may only be updated on their creating thread"
            ));
        }
        Ok(())
    }

    /// Runs one mutation inside GPUI's normal update/effect-flushing boundary.
    ///
    /// Returns an error when called from a different thread or reentrantly
    /// while the application is already mutably borrowed.
    pub fn update<R>(&self, update: impl FnOnce(&mut App) -> R) -> Result<R> {
        self.assert_main_thread()?;
        let mut app = self
            .app
            .try_borrow_mut()
            .context("embedded GPUI application is already being updated")?;
        Ok(app.update(update))
    }

    /// Returns GPUI's foreground executor.
    pub fn foreground_executor(&self) -> ForegroundExecutor {
        self.app.borrow().foreground_executor.clone()
    }

    /// Returns whether a retained window is dirty, needs presentation, or has
    /// an animation callback waiting for the next host frame.
    pub fn window_needs_frame(&self, handle: AnyWindowHandle) -> Result<bool> {
        self.assert_main_thread()?;
        let app = self
            .app
            .try_borrow()
        ...
        Ok(app
            ...
            .is_some_and(Window::needs_frame))
    }

    /// Gracefully shuts down GPUI entities, windows, and quit observers.
...
impl Application {
    #[cfg(test)]
    fn from_test_app_cell(app: Rc<AppCell>) -> Self {
        Self(app)
    }
    ...
    /// Starts GPUI without entering the platform event loop.
    ///
    /// The caller must retain the returned handle and drive foreground work
    /// through its host integration. The launch callback runs inside the same
    /// update/effect-flushing boundary as later [`EmbeddedApplication::update`]
    /// calls.
    pub fn start_embedded<F>(self, on_finish_launching: F) -> Result<EmbeddedApplication>
    where
        F: 'static + FnOnce(&mut App),
    {
        let embedded = EmbeddedApplication {
            app: self.0,
            main_thread: std::thread::current().id(),
        };
        embedded.update(on_finish_launching)?;
        Ok(embedded)
    }
...
mod test {
    ...
    use crate::{AppContext, Application, TestAppContext};

    #[test]
    fn embedded_application_has_explicit_update_and_reentrancy_boundaries() {
        let test_context = TestAppContext::single();
        let application = Application::from_test_app_cell(test_context.app.clone());
        let launches = Rc::new(RefCell::new(0));

        let embedded = application
            .start_embedded({
                let launches = launches.clone();
                move |_| *launches.borrow_mut() += 1
            })
            .unwrap();
        ...
        embedded
            .update(|_| {
                assert!(embedded.update(|_| ()).is_err());
            })
            .unwrap();
